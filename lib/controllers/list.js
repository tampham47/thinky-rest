...
List.prototype._applyOrderBy = function(query, sortQuery, alreadyUsingSecondaryIndex) {
  ...
  while (properties.length) {
    ...
    if (this.model._indexes.hasOwnProperty(property) && orderByOptions === null &&
        !alreadyUsingSecondaryIndex) {
...
List.prototype.fetch = function(req, res, context) {
  ...
  var alreadyUsingSecondaryIndex = false;
  if (!_.isEmpty(req.query) || !_.isEmpty(criteria)) {
    ...
    if (!!indexFilters.length) {
      ...
      if (!isNegated) {
        ...
        alreadyUsingSecondaryIndex = true;
      }
    }
  ...
  if (!!sortQuery) query = this._applyOrderBy(query, sortQuery, alreadyUsingSecondaryIndex);
  ...
  // execute queries
  var self = this;
  return Promise.all([ query.getJoin(joinProp).run(), countQuery.count().execute() ])
